...
生成统一的 primary CZ/中线 ROI 十面板图：

```matlab
run('scripts/systematic/config_02A.m')
run('scripts/systematic/plot_phase06_primary_cz_roi.m')
```

图中上排为 CZ，下排为 FZ/FCZ/CZ/CPZ ROI；HC 蓝、LC 红、HC−LC 黑，负向朝上。PNG 与可编辑 FIG 自动保存至该被试的 `derivatives/erp/<ID>/`。
