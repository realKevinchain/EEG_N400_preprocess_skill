...
% Fill only for channels judged globally bad before ICA.
cfg.bad_channels = [];
...
% Project-wide reference rule: average M1/M2 whenever both mastoids are
% usable. A single-mastoid reference is allowed only as a documented
% participant-specific exception.
...
cfg.icatrain_set = sprintf('%s_icatrain.set',cfg.subject);
