%% 828update Phase 4: ICA training, rank control, review, and cleaning.
...
thresholdQCPath = fullfile(cfg.qc_dir,sprintf( ...
    '%s_828update_%s_phase04_threshold_qc.csv', ...
    cfg.subject,cfg.reference_tag));
logPath = fullfile(cfg.logs_dir,sprintf( ...
    '%s_828update_%s_phase04_pass.txt',cfg.subject,cfg.reference_tag));
...
if all(completePresent)
    fprintf('828update Phase 4 already complete: %s\n',cleanPath);
    return
end
...
if exist(trainingPath,'file') == 0
    ...
    for trial = 1:cfg.expected_trials
        ...
        targetOffset = find(codes(startItem+1:end) == expectedTarget,1);
        assert(~isempty(targetOffset),'Missing target after trial start %d.',trial);
        ...
        segment = double(EEG.data(cfg.ica_channels,firstPoint:lastPoint));
        ...
        peakChannel(trial) = string(EEG.chanlocs( ...
            cfg.ica_channels(channelPosition)).labels);
    ...
    X = double(EEG.data(cfg.ica_channels,:));
    X = X-mean(X,2);
    numericalRank = rank(X);
    clear X
    expectedRank = numel(cfg.ica_channels);
    if strcmp(cfg.reference_mode,'average_mastoid') && ...
            all(ismember([cfg.m1_channel cfg.m2_channel],cfg.ica_channels))
    ...
    EEG.etc.update828_ica_training = struct( ...
        ...
        'ica_channels',cfg.ica_channels,'numerical_rank',numericalRank, ...
        'expected_rank',expectedRank,'reference_mode',cfg.reference_mode);
    EEG.setname = erase(cfg.icatrain_set,'.set');
    n400u_assert_dataset_absent(trainingPath);
    EEG = pop_saveset(EEG,'filename',cfg.icatrain_set,'filepath',cfg.ica_dir);
    fprintf(['828update ICA training copy created: rejected %d/%d task ' ...
        'segments; rank=%d/%d.\n'],sum(thresholdRejected), ...
        cfg.expected_trials,numericalRank,numel(cfg.ica_channels));
...
if exist(solutionPath,'file') == 0
    ...
    training = EEG.etc.update828_ica_training;
    assert(isequal(double(training.ica_channels),double(cfg.ica_channels)));
    numericalRank = double(training.numerical_rank);
    rng(cfg.ica_random_seed,'twister');
    arguments = {'icatype','runica','extended',1,'chanind',cfg.ica_channels};
    if numericalRank < numel(cfg.ica_channels)
...
if exist(weightsPath,'file') == 0
    ...
    assert(strcmp(ICA.etc.update828_ica_training.reference_mode, ...
        EEG.etc.update828_reference.mode));
    EEG.icaweights = ICA.icaweights;
    EEG.icasphere = ICA.icasphere;
    EEG.icawinv = ICA.icawinv;
    EEG.icachansind = ICA.icachansind;
    EEG.icaact = [];
    EEG.etc.update828_ica_training = ICA.etc.update828_ica_training;
...
EEG.etc.update828_ica_rejection = struct( ...
...
n400u_write_stage_log(cfg,'phase04',{ ...
    ...
    sprintf('ICA channels: %s',mat2str(cfg.ica_channels)), ...
    sprintf('Numerical rank: %d', ...
        EEG.etc.update828_ica_training.numerical_rank), ...
    sprintf('Threshold rejected trials: %s',mat2str( ...
        EEG.etc.update828_ica_training.threshold_rejected_trials)), ...
    sprintf('Removed ICs: %s',mat2str(removed)), ...
    sprintf('Output: %s',cleanPath)});
fprintf('828update Phase 4 PASS: removed ICs=%s; saved %s.\n', ...
