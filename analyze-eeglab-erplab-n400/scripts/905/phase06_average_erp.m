%% 905 Phase 6: behavior ledger, two ERPs, reload QC, and plots.
...
logPath = fullfile(cfg.logs_dir,sprintf( ...
    '%s_905_%s_phase06_pass.txt',cfg.subject,cfg.reference_tag));
primaryPNG20 = fullfile(cfg.erp_dir,sprintf( ...
    '%s_905_%s_erp_primary_correct_clean_cz_roi_scale20uv.png', ...
    cfg.subject,cfg.reference_tag));
primaryFIG20 = replace(primaryPNG20,'.png','.fig');
primaryPNG10 = replace(primaryPNG20,'scale20uv','scale10uv');
primaryFIG10 = replace(primaryPNG10,'.png','.fig');
allcleanPNG20 = fullfile(cfg.erp_dir,sprintf( ...
    '%s_905_%s_erp_all_clean_cz_roi_scale20uv.png', ...
    cfg.subject,cfg.reference_tag));
allcleanFIG20 = replace(allcleanPNG20,'.png','.fig');
allcleanPNG10 = replace(allcleanPNG20,'scale20uv','scale10uv');
allcleanFIG10 = replace(allcleanPNG10,'.png','.fig');
outputs = {ledgerPath,binSummaryPath,primaryERPPath,allcleanERPPath, ...
    primaryPNG20,primaryFIG20,primaryPNG10,primaryFIG10, ...
    allcleanPNG20,allcleanFIG20,allcleanPNG10,allcleanFIG10,logPath};
present = cellfun(@(p) exist(p,'file') == 2,outputs);
if all(present)
    fprintf('905 Phase 6 already complete: %s\n',logPath);
    return
end
...
ERP_ALLCLEAN = pop_averager(EEG,'Criterion','good', ...
    'ExcludeBoundary','on','SEM','on','Warning','off','DQ_flag',1);
ERP_PRIMARY.erpname = sprintf('%s_905_primary_correct_clean',cfg.subject);
ERP_ALLCLEAN.erpname = sprintf('%s_905_all_clean',cfg.subject);
...
plot_905_erp_qc(cfg,PRIMARY_CHECK,ALLCLEAN_CHECK);
n400u_write_stage_log(cfg,'phase06',{ ...
    ...
    sprintf('EEG artifact rejection rate: %.2f%%',100*mean(artifact)), ...
    sprintf('Primary exclusion rate: %.2f%%',100*mean(~primaryGood)), ...
    'Single-word plot scales: fixed +/-20 uV and fixed +/-10 uV', ...
    'ERP save/reload differences: 0', ...
    sprintf('Primary ERP: %s',primaryERPPath), ...
    sprintf('All-clean ERP: %s',allcleanERPPath)});
fprintf('905 Phase 6 PASS: primary accepted per bin = ');
